...
static Vector *read_args(Token *tok, Macro *macro) {
    ...
    if (vec_len(args) != macro->nargs)
        errort(tok, "kycc: macro argument doesnt verify.");
    return args;
}
...
static bool read_funclike_macro_params(Token *name, Map *param) {
    ...
    for (;;) {
        ...
        if (pos) {
            if (!is_keyword(tok, ','))
                errort(tok, "kycc: , expected, but got %s", tok2s(tok));
            tok = lex();
        }
        if (tok->kind == TNEWLINE)
            errort(name, "kycc: missing ')' in macro parameter list");
        ...
        if (tok->kind != TIDENT)
            errort(tok, "kycc: identifier expected, but got %s", tok2s(tok));
...
static void hashhash_check(Vector *v) {
    ...
    if (is_keyword(vec_head(v), KHASHHASH))
        errort(vec_head(v), "kycc: '##' cannot appear at start of macro expansion");
    if (is_keyword(vec_tail(v), KHASHHASH))
        errort(vec_tail(v), "kycc: '##' cannot appear at end of macro expansion");
}
...
static char *read_cpp_header_name(Token *hash, bool *std) {
    ...
    Token *tok = read_expand_newline();
    if (tok->kind == TNEWLINE)
        errort(hash, "kycc: expected filename, but got newline");
    ...
    for (;;) {
        ...
        if (tok->kind == TNEWLINE)
            errort(hash, "kycc: premature end of header name");
...
static bool guarded(char *path) {
    ...
    define_obj_macro("__kycc_include_guard", r ? cpp_token_one : cpp_token_zero);
...
static void handle_pragma_macro(Token *tmpl) {
    ...
    if (operand->kind != TSTRING)
        errort(operand, "kycc: _Pragma takes a string literal, but got %s", tok2s(operand));
...
static void init_predefined_macros() {
    ...
    vec_push(std_include_path, "/usr/local/lib/kycc/include");
    ...
    read_from_string("#include <" BUILD_DIR "/include/kycc.h>");
}
...
Token *read_token() {
    ...
    for (;;) {
        ...
        // Against token directive to test useability
