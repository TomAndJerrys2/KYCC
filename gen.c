...
static char *get_int_reg(Type *ty, char r) {
    ...
    switch (ty->size) {
        ...
        default:
            error("kycc: Unknown data size: %s: %d", ty2s(ty), ty->size);
    }
}
...
static void emit_assign_struct_ref(Node *struc, Type *field, int off) {
    ...
    switch (struc->kind) {
        ...
        case AST_DEREF:
            ...
            break;
        default:
            error("kycc: internal error: %s", node2s(struc));
    }
}
...
static void emit_store(Node *var) {
    ...
    switch (var->kind) {
        ...
        default: error("kycc: internal error");
    }
}
...
static void emit_binop(Node *node) {
    ...
    if (is_inttype(node->ty))
        ...
        error("kycc: internal error: %s", node2s(node));
}
...
static void emit_save_literal(Node *node, Type *totype, int off) {
    switch (totype->kind) {
        ...
        default:
            error("kycc: internal error: <%s> <%s> <%d>", node2s(node), ty2s(totype), off);
        }
}
    ...
    case KIND_PTR:
        ...
        if (is_char_ptr) {
            ...
            if (base->kind != AST_GVAR)
                error("kycc: global variable expected, but got %s", node2s(base));
            assert(ty->ptr);
            emit(".quad %s+%u", base->glabel, v * ty->ptr->size);
        }
        break;
    default:
        error("kycc: don't know how to handle\n  <%s>\n  <%s>", ty2s(ty), node2s(val));
    }
}
