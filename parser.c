...
static void ensure_lvalue(Node *node) {
    switch (node->kind) {
        case AST_LVAR: case AST_GVAR: case AST_DEREF: case AST_STRUCT_REF:
            return;
        default:
            error("kycc: lvalue expected, but got %s", node2s(node));
    }
}

static void ensure_inttype(Node *node) {
    if (!is_inttype(node->ty))
        error("kycc: integer type expected, but got %s", node2s(node));
}

static void ensure_arithtype(Node *node) {
    if (!is_arithtype(node->ty))
        error("kycc: arithmetic type expected, but got %s", node2s(node));
}

static void ensure_not_void(Type *ty) {
    if (ty->kind == KIND_VOID)
        error("kycc: void is not allowed");
}
...
static Node *conv(Node *node) {
    ...
    switch (ty->kind) {
        case KIND_ARRAY:
            ...
            // C11 An array of T is converted to a pointer to T.
            return ast_uop(AST_CONV, make_ptr_type(ty->ptr), node);

        case KIND_FUNC:

            // C11 A function designator is converted to a pointer to the function.
            return ast_uop(AST_ADDR, make_ptr_type(ty), node);

        case KIND_SHORT: case KIND_CHAR: case KIND_BOOL:
            // C11 The integer promotions
...
static void ensure_assignable(Type *totype, Type *fromtype) {
    ...
    error("kycc: incompatible kind: <%s> <%s>", ty2s(totype), ty2s(fromtype));
}
...
int eval_intexpr(Node *node, Node **addr) {
    ...
    #undef R
        default:
            error:
                error("kycc: Integer expression expected, but got %s", node2s(node));
            }
}
...
static Node *read_primary_expr() {
    ...
    switch (tok->kind) {
        ...
        case TKEYWORD:
            ...
            return NULL;
        default:
            error("kycc: internal error: unknown token kind: %d", tok->kind);
    }
}
...
static int read_bitsize(char *name, Type *ty) {
    if (!is_inttype(ty))
        error("kycc: non-integer type cannot be a bitfield: %s", ty2s(ty));
    Token *tok = peek();
    int r = read_intexpr();
