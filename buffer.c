...
// * Initialize a Buffer Call and Allocation
Buffer *make_buffer() {
...
static void realloc_body(Buffer *b) {
    ...
    // This will eventually be changes to the ElivyianC version
...
// * Print Buffer to IO using blocks *
...
void buf_printf(Buffer *b, char *fmt, ...) {
    ...
    for (;;) {
        ...
        // check if reallocation is successful on a bigger set
        if (avail <= written) {
            realloc_body(b);
...
// Check quotes for escape/terminal-sequence characters
...
// * Trace for the buffer created for the c-string 
